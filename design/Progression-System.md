# Progression System

How characters, classes, and abilities develop across all 64 chapters of Vigilans Nexum.

---

## 🎯 Overall Progression Arc

**Total Game Length:** 64 Chapters across 8 Parts
**Chapters per Unit:** 52 progression chapters – Parts 01–04 (32) + one strand of Part 05/06 (8) + Part 07 (8) + Tower (4). The epilogue (Ch 53–56) grants no XP.
**Level Range:** 1 → 60, cap reached at the Dajjal (Ch 52)
**Pace:** ~1.13 levels per progression chapter
**Class Tiers:** Citizen → Base → Intermediate → Advanced → Master (Dardan and Hasan add a Lord Kit on top of Master)

**Design intent:** The eight Vigilant Knights are the only units that start at Level 1 in the Citizen class. Everyone else joins at their established experience level. Over 59 level-ups the Bellum orphans overtake the veterans who joined ahead of them – chosen family outgrowing inherited status, expressed as a stat sheet rather than a line of dialogue.

---

## 📊 Level Curve per Part

| Part | Chapters | Avg Level | Tier Event |
|------|----------|-----------|------------|
| 01 Path of Liberation | 01–08 | 1 → 11 | **Base class chosen at the end of Ch 06** · promotion teaser Ch 08 |
| 02 A King's Promise | 09–16 | 11 → 20 | **Intermediate at Lv 15** |
| 03 The Champion's Road | 17–24 | 20 → 29 | Intermediate phase |
| 04 The Name You Were Given | 25–32 | 29 → 37 | **Advanced at Lv 30** |
| 05 *or* 06 (parallel strand) | 33–40 | 37 → 45 | Advanced – identical band in both strands |
| 07 Before The Storm | 41–48 | 45 → 54 | **Master at Lv 45** – mass promotion at the reunion; Lord Kit for Dardan and Hasan |
| 08 The Tower | 49–52 | 54 → **60** | Cap reached at the Dajjal |
| 08 Epilogue | 53–56 | 60, static | No XP – support ranks max out here |

---

### Part 01: Path of Liberation (Chapters 1-8)

**Level Range:** 1 → 11
**Enemy Levels:** 1 → 13
**Focus:** Learning fundamentals, establishing base roster

| Chapter | Avg Level Start | Avg Level End | New Units | Mechanics Introduced |
|---------|-----------------|---------------|-----------|----------------------|
| 01 | 1 | 2 | Dardan, Hasan | Movement, Attack, Items |
| 02 | 2 | 4 | Maksimo | Weapon Triangle |
| 03 | 4 | 5 | Ivan, Leona | Terrain Effects, Objectives |
| 04 | 5 | 6 | - | Naval Combat (Optional) · Weapon delivery – all physical weapon types |
| 05 | 6 | 8 | Lina, Marven | Magic Introduction · one tome per element found |
| 06 | 8 | 9 | - | Support Conversations · **Base class chosen at chapter end** |
| 07 | 9 | 10 | Mikael | The Nexus / Exchange · **first chapter played in the chosen classes** |
| 08 | 10 | 11 | - | Boss Battle, Promotion Preview |

**Why the class choice sits at the end of Ch 06:** By that point the player has seen movement, the weapon triangle, terrain, objectives, naval combat, magic (Ch 05) and support conversations. The first irreversible decision of the campaign is therefore made with the systems already demonstrated rather than guessed at – and Ch 07 is immediately there to play the result.

**Milestone:** By end of Part 01, all eight Vigilant Knights are recruited.

---

### Part 02: A King's Promise (Chapters 9-16)

**Level Range:** 11 → 20
**Enemy Levels:** 13 → 22
**Focus:** Class specialization, expanding roster with royal allies

| Chapter | Avg Level Start | Avg Level End | New Units | Mechanics Introduced |
|---------|-----------------|---------------|-----------|----------------------|
| 09 | 11 | 12 | Tifa, Claude | Mounted Units |
| 10 | 12 | 13 | - | Shopping/Economy |
| 11 | 13 | 15 | Fabius | Heavy Armor Units |
| 12 | 15 | 16 | Asta | Divine Magic (Lux) · **Intermediate classes unlock** |
| 13 | 16 | 17 | Vector | Assassin Archetype |
| 14 | 17 | 18 | Shira | Advanced Bow Techniques |
| 15 | 18 | 19 | - | Multi-Phase Battle |
| 16 | 19 | 20 | - | Major Story Boss |

**Milestone:** Kingdom of Adolla arc complete, Tifa joins permanently.

---

### Part 03: The Champion's Road (Chapters 17-24)

**Level Range:** 20 → 29
**Enemy Levels:** 22 → 31
**Focus:** Mastery of combat arts, arena challenges

| Chapter | Avg Level Start | Avg Level End | New Units | Mechanics Introduced |
|---------|-----------------|---------------|-----------|----------------------|
| 17 | 20 | 21 | - | Naval Battle System |
| 18 | 21 | 22 | - | Ghost Ship (Horror Theme) |
| 19 | 22 | 24 | Kassandra | Thysian Combat Arts |
| 20 | 24 | 25 | - | Darkness Magic (Umbra) |
| 21 | 25 | 26 | Rohan | Desert Survival Mechanics |
| 22 | 26 | 27 | - | Sandstorm Hazards |
| 23 | 27 | 28 | - | Arena Tournament (Multi-Battle) |
| 24 | 28 | 29 | - | Dardan's Awakening (Story Event) |

**Also joining in Part 03:** **Alex** and **Raphael**. Their exact recruitment chapter is still open – neither has a character sheet yet, so the table above leaves their slot unassigned.

**Milestone:** Thysia arc complete, Dardan recognized as Champion of Aurevia.

---

### Part 04: The Name You Were Given (Chapters 25-32)

**Level Range:** 29 → 37
**Enemy Levels:** 31 → 39
**Focus:** Multi-army battles, civil war, imperial conflict

| Chapter | Avg Level Start | Avg Level End | New Units | Mechanics Introduced |
|---------|-----------------|---------------|-----------|----------------------|
| 25 | 29 | 30 | Elena, Matteo | Imperial Tactics · **Advanced classes unlock** |
| 26 | 30 | 31 | - | Civil War Mechanics |
| 27 | 31 | 32 | - | Multi-Army Battles |
| 28 | 32 | 33 | Luca | Political Choices |
| 29 | 33 | 35 | Rael | Enemy Conversion |
| 30 | 35 | 36 | - | Allied NPC Armies |
| 31 | 36 | 36 | - | Pre-Finale Battle |
| 32 | 36 | 37 | - | Part Finale (Multi-Phase) |

**Milestone:** The party splits in Valdris – Part 05 and Part 06 begin.

---

### Part 05 / Part 06: The Parallel Strands (Chapters 33-40)

**Level Range:** 37 → 45 – **identical in both strands**
**Enemy Levels:** 39 → 47 – **must be identical in both strands**
**Focus:** Split roster, two independent 8-chapter campaigns

| Strand | Chapters | New Units |
|--------|----------|-----------|
| Part 05 – Divided We Face Our Sins (Dardan, Adolla) | 33A–40A | Jessy Harvel, Aldric Halvorn, Edric Thalven, Maluna Eloise |
| Part 06 – Divided We Find Our Virtue (Hasan, Vaestrall) | 33B–40B | Aylin Vaestrall, Rijona Alior, Corvan Gault, Maike Collis |

**Why no special XP rule is needed:** Both strands are exactly eight chapters long and share one level band. As long as enemy levels match, both groups arrive in Part 07 at the same strength. The split resolves itself.

**What is needed instead – catch-up XP:** With 30+ recruited units and 14–16 deployment slots, the spread *within* a group is larger than the spread *between* the groups. Units below their group's average level gain bonus XP so the bench does not fall out of the game.

**Milestone:** Both groups reach Lv 45 at the end of their strand and reunite in Ch 41.

---

### Part 07: Before The Storm (Chapters 41-48)

**Level Range:** 45 → 54
**Enemy Levels:** 47 → 56
**Focus:** Reunited roster, mass promotion, preparation for the tower

| Chapter | Avg Level Start | Avg Level End | Mechanics Introduced |
|---------|-----------------|---------------|----------------------|
| 41 | 45 | 47 | Reunion · **Master classes unlock** · Lord Kit for Dardan and Hasan |
| 42 | 47 | 48 | – |
| 43 | 48 | 49 | Varnel / Dajjal revelation (Story Event) |
| 44 | 49 | 50 | – |
| 45 | 50 | 51 | – |
| 46 | 51 | 52 | War Council, allied armies |
| 47 | 52 | 53 | Naval approach |
| 48 | 53 | 54 | Arrival at the tower |

**New Units:** Three recruitment encounters exist in the written chapters (Verna, the temple monks, Blackbeard) but have no character sheets yet – see `Pacing-Audit.md`.

**Why the Master gate sits at the reunion:** Both split groups hit Lv 45 at the end of their strand. The last promotion of the game therefore happens the moment the party that separated in Valdris stands together again – one mechanical beat and one narrative beat in the same scene.

---

### Part 08: The Thread That Holds (Chapters 49-56)

#### The Tower (Chapters 49-52) – four battles

**Level Range:** 54 → 60 (cap)
**Enemy Levels:** 56 → 62, all promoted
**Pace:** ~1.5 levels per chapter – justified by promoted enemies (+5 XP) and boss kills (×3 XP)

| Chapter | Avg Level Start | Avg Level End | Opposition |
|---------|-----------------|---------------|------------|
| 49 They Forgot The Thread | 54 | 56 | Mordain, Leander, Cassius |
| 50 That Once Bound Them All | 56 | 57 | Gentian & Kresnik |
| 51 The Path Grew Dark | 57 | 58 | The seven sisters |
| 52 And The Lights Did Fall | 58 | **60** | The Dajjal |

**Ch 52 is the last deployment decision of the game** – who fights the Dajjal is chosen here.

#### The Epilogue (Chapters 53-56) – no progression

**Level:** 60, static. No XP, no promotions, no seals. Every unit's class is final long before this point.

The journey through Adolla, Thysia and Vaestrall, gathering people until all roads lead to Bellum. The Nexus bond is finished as a combat tool at Ch 52; here it pays off as a relationship. **Support ranks reach their maximum in these four chapters** – that is their mechanical purpose.

> **Permadeath consideration:** `Pacing-Audit.md` marks Ch 52 as the intended place for a lasting loss. Four epilogue chapters have to carry that loss without breaking their fixed text – whoever falls at the Dajjal must stay gone through the festival.

---

## 🏆 Class Progression System

### Class Tiers & Requirements

```
Citizen (Lv 1)
    ↓ [End of Chapter 06 – story event, no level requirement]
Base Classes (Swordsman, Archer, Pyromancer, ...)
    ↓ [Level 15 + Seal]
Intermediate Classes (Myrmidon, Sniper, Pyro Sage, ...)
    ↓ [Level 30 + Seal]
Advanced Classes (Sword Master, Assassin, Elementalist, ...)
    ↓ [Level 45 + Seal]
Master Classes (Sword Saint, Nightveil, Arcanist, ...)
    │
    └─ [Story unlock at Lv 45] → Lord Kit (Dardan and Hasan only)
                                  Master class + kit abilities, carrying the
                                  class's Lord Title (Aetherblade, Ashborn, ...)
```

**Unique is Master plus a Lord Kit, not a class of its own.** Dardan and Hasan promote into a Master class like everyone else – same gate, same seal, same terminal tier. What sets them apart is the Lord Kit: a set of story-unlocked abilities laid on top of the Master class, opening at Lv 45. The *Lord Title* is simply the Master class's name when Dardan or Hasan hold it – Sword Saint reads *Aetherblade*, Shadow Monarch reads *Ashborn* – and is cosmetic. Which Master class carries which title, and what each kit contains, is in [catalog/Unit-Classes.md](catalog/Unit-Classes.md).

That makes **four promotions** across the campaign, not five.

### Weapon Rank

Weapon ranks run **F–S** (F, E, D, C, B, A, S) per weapon type, and a weapon can only be equipped by a unit holding its rank in that type. How a rank rises (weapon experience through use is the Fire Emblem convention) is not yet specified. The **rank cap rises with class tier**: each promotion opens the next span of ranks, and **S opens only at Master** – the last promotion of the game is also the last thing a weapon rank can give. A hybrid class's secondary weapon types (every type after the first in *Weapon Types*, e.g. the Duelist's Knife, the Tenebrae's Sword, the Priest's Lux) cap **one step below the main type** – a hybrid pays for its breadth in depth, so the single-type branch at the same fork is never strictly worse. **Rank F exists only in the Citizen class** – the eight Vigilant Knights are the only units that ever hold it; every other unit joins in a base class or higher and starts at E or above. A Citizen holds F in **every weapon type**, reached in three stages – Sword, Lance, Axe, Bow and Staff from Ch 01, the remaining physical types with the Ch 04 weapon delivery, the nine elements with the tomes found in Ch 05 – so that every type has been tried before the Ch 06 choice; the staging and the Citizen's MP regime by equipped weapon are in [Growth Modifiers](mechanics/Growth-Modifiers.md#core-rules).

The caps per tier are in the [Balancing Guide → Weapon Rank Caps](Balancing-Guide.md#weapon-rank-caps).

**Healing with a Staff grants XP.** A Staff is a weapon type, so a healer levels like any other unit – through its weapon, not through kills. The value is the heal entry in the [XP Gain Formula](Balancing-Guide.md#xp-gain-formula).

### Promotion Requirements

**Level and a seal – nothing else.** There are no stat requirements. A unit that reaches the level can always take the promotion, and no unit can ever be locked out of its branch by unlucky growth rolls. The first step is not even level-gated: the base class is chosen in a story event, so no unit can arrive in Part 02 still a Citizen.

| Tier | Requirement | Seal Cost |
|------|-------------|-----------|
| Base | End of Chapter 06 (story event) | Free |
| Intermediate | Level 15 | 500 Gold |
| Advanced | Level 30 | 2,000 Gold |
| Master | Level 45 | 5,000 Gold |
| Lord Kit | Story unlock at Lv 45, on top of the Master promotion (Dardan/Hasan) | Cannot buy |

Seals are **promotion items, not reclassing items** – each one is consumed to move a unit one tier up its own branch. They cannot move a unit sideways into a different class line.

**Why no stat gates:** With permanent promotions, a stat requirement would be a second irreversible failure mode – a unit could miss its gate through bad RNG and stay stuck a tier below the rest of the army for the remaining campaign, with no way to correct it. Level alone is the honest gate: it measures what the player invested, not what the dice granted.

### Class Change Rules

- **Promotion is permanent.** There is no reclassing. A unit moves up its branch and never sideways or back – once chosen, a class is that unit's identity for the rest of the campaign.
- **Every promotion is a fork.** The class tree in [catalog/Unit-Classes.md](catalog/Unit-Classes.md) branches at every tier: Citizen opens into any base class, Myrmidon leads to Sword Master, Duelist *or* Griffon Knight, Soldier to Halberdier, Armored Knight *or* Pegasus Knight. Four promotions per unit means up to four irreversible decisions.
- **Skill Retention:** Abilities learned in a lower class are kept after promoting.
- **Level Retention:** Level stays the same across a promotion – levels are continuous 1–60, not per tier.
- **Stat Adjustments:** Class bonuses apply immediately.
- **Growth Adjustments:** Every class carries a growth modifier per stat that is added to the unit's personal growth on each level-up spent in that class; it rises with tier and is 0 for the Citizen. The rule is in [Growth Modifiers](mechanics/Growth-Modifiers.md), the values in the [Balancing Guide → Class Growth Modifiers](Balancing-Guide.md#class-growth-modifiers).

**Consequence for the UI:** Because the choice cannot be undone, the branch options, their stat modifiers *and their growth modifiers* must be fully visible *before* the player commits. Design Pillar 5 demands the challenge be transparent – an irreversible decision made on hidden information would be punishment, not challenge.

**Consequence for balancing:** No branch may be a trap. A unit promoted down a weaker branch has no recovery path, so every option at a fork needs a niche where it is the right answer – see the "no strictly better options" principle in the [Balancing Guide](Balancing-Guide.md).

### Class Tier Stat Modifiers

See [Balancing Guide → Class Balancing](Balancing-Guide.md#-class-balancing) – all stat modifiers and the class growth modifiers live there.

---

## 🌟 Recruitment Timeline

### Joining Level by Part

New units join slightly below the party average so they are useful without outclassing units the player has invested in.

| Joins in | Joining Level | Tier | Units |
|----------|---------------|------|-------|
| Part 01 | **1** | Citizen | Dardan, Hasan, Maksimo, Ivan, Leona, Lina, Marven, Mikael |
| Part 02 | 10–15 | Base | Tifa, Claude, Fabius, Asta, Vector, Shira |
| Part 03 | 19–24 | Base / Intermediate | Kassandra, Rohan, Alex, Raphael |
| Part 04 | 28–33 | Intermediate | Elena, Matteo, Luca, Rael |
| Part 05 | 36–41 | Intermediate / Advanced | Jessy, Aldric Halvorn, Edric, Maluna |
| Part 06 | 36–41 | Intermediate / Advanced | Aylin, Rijona, Corvan, Maike |
| Part 07 | 44–49 | Advanced | Verna, temple monks, Blackbeard *(no sheets yet)* |

**Roster total:** 30 named playable units – 28 with character sheets, plus Alex and Raphael (Part 03) still to be written. The Part 07 recruits are additional and unnamed so far.

Late joiners need high base stats to be viable – they have far fewer growth rolls left. A Part 07 recruit gets 18 level-ups; a Vigilant Knight gets 59. That gap is the point, not a bug.

### Deployment Limits by Chapter

| Chapters | Max Deployment | Notes |
|----------|----------------|-------|
| 01-04 | 4-6 units | Forced deployment (tutorial) |
| 05-08 | 8 units | Flexibility begins |
| 09-16 | 10 units | Standard mid-game |
| 17-32 | 12 units | Larger battles |
| 33-40 | 8-10 units | **Split roster** – smaller pool per strand |
| 41-48 | 14 units | Reunited army |
| 49-52 | 16 units | Endgame, tower |
| 53-56 | – | Epilogue, no deployment |

**Special:** Some chapters force specific units (story reasons).

---

## 📈 Stat Growth Across Game

### Expected Stats at Key Milestones (Balanced Unit, ~350% growth budget)

| Level | HP | MP | Str | Mag | Dex | Spd | Lck | Def | Res |
|-------|-----|-----|-----|-----|-----|-----|-----|-----|-----|
| **1** (Citizen) | 18 | 10 | 5 | 3 | 5 | 5 | 4 | 4 | 2 |
| **15** (Intermediate) | 26 | 14 | 11 | 6 | 11 | 11 | 9 | 9 | 6 |
| **30** (Advanced) | 35 | 19 | 18 | 9 | 18 | 18 | 14 | 14 | 9 |
| **45** (Master) | 44 | 23 | 25 | 12 | 25 | 25 | 19 | 19 | 13 |
| **60** (Cap) | 53 | 28 | 32 | 15 | 32 | 32 | 25 | 25 | 17 |

*Values before class bonuses. A Master-tier unit adds roughly +10 HP, +20 MP, +5 Str/Mag, +4 Spd, +4 Def/Res on top, landing near 63 HP / 37 Str – comfortably under the caps of 90 HP / 50, with headroom for min-maxed units and stat boosters.*

See [Balancing Guide](Balancing-Guide.md) for growth rate budgets and absolute caps.

---

## ⚔️ Weapon Progression Timeline

| Tier | First Available | Typical Use Period | Obsolete By |
|------|----------------|-------------------|-------------|
| **Iron** | Chapter 1 | Ch 1-16 | Never (budget option) |
| **Steel** | Chapter 9 | Ch 9-32 | Never (mid-game standard) |
| **Silver** | Chapter 25 | Ch 25-52 | Never (expensive but reliable) |
| **Legendary** | Chapter 41 | Ch 41-52 | Endgame only |
| **Unique/Prf** | Story-based | Character-specific | Never |

### Magic Tome Availability

*Proposal – the rank-to-tier mapping is a first draft for tuning.* Tomes are weapons and use the same tiers as the table above and the same F–S ranks as every weapon type (see [Weapon Rank](#weapon-rank)). Rows are ranks; the class tier that first holds a rank follows from the [Weapon Rank Caps](Balancing-Guide.md#weapon-rank-caps), the chapter from the tier gates above.

| Rank | First held in | From chapter | Tome tier available |
|------|---------------|--------------|---------------------|
| F | Citizen | Ch 05 | Bronze – one tome per element, found, not bought |
| E | Base | Ch 06 | Iron |
| D | Base | Ch 06 | Steel |
| C | Intermediate | Ch 12 | Steel |
| B | Intermediate | Ch 12 | Steel |
| A | Advanced | Ch 25 | Silver |
| S | Master | Ch 41 | Legendary |

---

## 🎓 Skill & Ability Unlocks

**Passive Abilities:** Learned through class mastery
**Combat Arts:** Learned at specific class levels, cost MP
**Magic Tomes:** Equipped like weapons, gated by magic rank – no spell is learned
**Unique Skills:** Story-based unlocks

### Sample Ability Timeline (Dardan)

| Level | Class | Ability/Skill Unlocked |
|-------|-------|------------------------|
| 1 | Citizen | Adaptability, Discipline, Aptitude |
| 9 (Ch 06) | Swordsman | Speed +2 |
| 15 | Myrmidon | Vantage |
| 22 | Myrmidon | Wrath |
| 30 | Sword Master | Sol |
| 38 | Sword Master | Swordfaire |
| 45 | Aetherblade (Unique) | **Nexus Mastery** (Exchange has no cooldown) |
| 54 | Aetherblade | **Bond of Souls** (Adjacent allies +5 all stats) |

---

## 💎 Resource Progression

### Gold Economy Over Time

| Part | Avg Gold Earned | Cumulative Total | Major Purchases |
|------|-----------------|------------------|-----------------|
| 01 | 6,000 | 6,000 | Base weapons, first seals |
| 02 | 12,000 | 18,000 | Steel weapons, intermediate seals |
| 03 | 16,000 | 34,000 | Combat arts, equipment |
| 04 | 20,000 | 54,000 | Silver weapons, advanced seals |
| 05 / 06 | 12,000 per strand | 78,000 | Split convoy – both strands earn |
| 07 | 24,000 | 102,000 | Master seals, legendary upkeep |
| 08 (Tower) | 15,000 | 117,000 | Stat boosters, final loadout |
| 08 (Epilogue) | – | 117,000 | No income |

### Stat Booster Distribution

**Energy Drop (HP+5), Spirit Dust (Mag+2), etc.**

- Part 01: 1-2 (hidden on maps)
- Part 02: 3-4
- Part 03: 4-5
- Part 04: 5-6
- Part 05/06: 4-5 per strand
- Part 07: 6-8
- Part 08 (Tower): 4-6, plus limited purchase stock

---

## 🎮 Difficulty Curve

```
Ch 01-03:  Easy         (Tutorial, learning mechanics)
Ch 04-07:  Medium       (Applying knowledge)
Ch 08:     Spike        (First major boss)
Ch 09-15:  Medium       (New mechanics, new difficulty)
Ch 16:     Spike        (Part 02 finale)
Ch 17-23:  Medium-Hard  (Optimization required)
Ch 24:     Spike        (Arena + story boss)
Ch 25-31:  Hard         (Mastery expected)
Ch 32:     Spike        (Part 04 finale, army splits)
Ch 33-40:  Hard         (Split roster – fewer units, same pressure)
Ch 41-47:  Very Hard    (Reunited army, larger maps)
Ch 48:     Spike        (Arrival at the tower)
Ch 49-51:  Very Hard    (Tower gauntlet)
Ch 52:     Extreme      (The Dajjal)
Ch 53-56:  –            (Epilogue, no combat)
```

**Note:** This is for "Normal" difficulty. Hard/Maddening increase by 1-2 tiers.

---

## ✅ Progression Checklist

Ensure each Part includes:

- [ ] 2-4 new playable units at the correct joining level for that Part
- [ ] Clear mechanical theme
- [ ] 1-2 major boss battles
- [ ] Story climax at Part finale
- [ ] Enemy levels 2 above the player average (Normal)

Additionally for Parts 05 and 06:

- [ ] Both strands use **identical** enemy level bands
- [ ] Both strands recruit the same number of units

---

**Version:** 2.1
**Last Updated:** 2026-09-18
**Cross-Reference:** [Balancing Guide](Balancing-Guide.md), [Game Mechanics](mechanics/README.md), [Unit Classes](catalog/Unit-Classes.md), [Levels](levels/README.md)
