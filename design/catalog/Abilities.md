...
| Disarray | 3 | Active (action) | Instead of attacking, choose an enemy within range 2: every enemy adjacent to that enemy is pushed one tile directly away from it. The chosen enemy itself does not move. No damage. A push fails against a unit whose Strength is above this unit's or whose tile beyond is blocked or impassable; the others are pushed regardless. What the Saboteur wrecks is the formation: Hold Fast, Shield Brother, Pit Fighter, Guard and every "while adjacent" rule lose their neighbour, the healer behind the wall is uncovered, an enemy chain attack is broken before it forms. Shove moves one body; Disarray moves everyone around one. Replaces *Sabotage* – an Advanced class held a stronger disable than the Lord's own seal, [Earthbound](../mechanics/Nexus.md). | Saboteur |
...
**Dardan's kit is the Nexus.** The six Nexus abilities are unlocked by chapter, not by level – the first arrives at Ch 08, long before the Master promotion; *Soulcairn* has no chapter at all and appears at the first permanent loss, and *Wavelength*'s chapter is not yet decided – and they are **not equipped and take no Capacity** (a Base class holds 3 and Exchange would not fit). Full rules in [The Nexus](../mechanics/Nexus.md); every value in the [Balancing Guide → The Nexus](../Balancing-Guide.md#nexus). *Active (command)* below is the trigger defined there: it ends the unit's movement for the turn but leaves its action. All six are unusable on a map whose level box locks the band (band shown black).

| Name | Capacity | Trigger | Effect | Class |
| ---- | -------- | ------- | ------ | ----- |
| Exchange | – | Active (command) | Trade places with one allied unit within the Exchange range for the affinity rank Dardan holds with it ([Affinity](../mechanics/Affinity.md)); the range goes through enemies, walls and barriers. Both landing tiles must be passable for the unit that lands on them; the ally's status is irrelevant. Afterwards Dardan may attack or use an item but not move; the ally keeps its action if unused. Global turn cooldown, flat – the rank scales the range, never the cooldown. Never a summoned beast. Unlocked Ch 08. | Lord Kit: Dardan |
| Lifeline | – | Active (command) | Link the band to one allied unit within the Lifeline range for its affinity rank. Until Dardan lifts it (free), the Lifeline share of every attack's damage that unit takes is taken from Dardan's HP instead – with no floor: it can kill him – and every status effect that unit would receive lands on Dardan instead, whole; a status Dardan is immune to fizzles. One at a time; holds at any distance once placed. Unlocked *chapter not yet decided*. | Lord Kit: Dardan |
| Heartpulse | – | Active (action) | Every allied unit on the map that holds an affinity rank with Dardan recovers HP, wherever it stands; how much, per unit, is set by that rank. **Once per chapter.** Not a staff: no weapon XP, no *on healing* triggers. Unlocked Ch 21. | Lord Kit: Dardan |
| Earthbound | – | Active (action) | Seal one **enemy** within a fixed range – the one Nexus ability that targets the other side. For the seal duration the target loses everything that runs through the band: all abilities, combat arts, tomes, staves and the auras it projects. It keeps the plain weapon strike, its movement and its items. No hit roll, no rank scaling; global turn cooldown. It closes, it never holds or controls – and it is not Geo magic. On the Dajjal it cuts the chain for its duration (Level 52). Unlocked Ch 44. | Lord Kit: Dardan |
| Wavelength | – | Active (command) | Choose one **living** allied unit within the range its affinity rank gives, and Dardan's ordinary attacks carry **that unit's elemental affinity** ([Affinity](../mechanics/Affinity.md#7--elemental-affinity-and-who-has-none)) for a number of attacks set by that rank – the [Magic System](../mechanics/Magic-System.md#wie-elemente-appliziert-werden)'s **one exception** to "a non-mage applies only through Combat Arts". **He applies, he never triggers:** a Wavelength attack onto an already applied element deals damage and leaves the element lying. The element is fixed when the command is given and does not change mid-combat; only the element is borrowed – no ability, art, stat or growth. Global turn cooldown. Unlocked *chapter not yet decided*. | Lord Kit: Dardan |
| Soulcairn | – | Passive | **Classic only.** Every playable unit that has fallen leaves its pair with Dardan frozen on the chart; the fallen together are **one partner** on his affinity list, named *Soulcairn*, whose points are the sum of those frozen pairs and whose rank is read from the ordinary [rank thresholds](../Balancing-Guide.md#rank-thresholds), capped at S. It grants the ordinary [affinity combat bonus](../mechanics/Affinity.md#4--the-combat-bonus) for that rank – **only when no living ranked partner is within the bonus range**; it fills the gap and never stacks, and a living partner at D beats it at S. Its element is that of the heaviest fallen unit and changes when a heavier one falls; ties go to the later death. No Capacity, no range, no cooldown, nothing to use – and nothing at all while nobody has been lost. Unlocked by the first Classic loss, in whichever chapter it happens. | Lord Kit: Dardan |
| Nexus Mastery | 4 | Passive | Every Nexus cooldown is removed – Exchange and Earthbound – and the Exchange and Lifeline ranges grow at every affinity rank by the bonus in the [Balancing Guide → Nexus Mastery](../Balancing-Guide.md#nexus-mastery). Heartpulse stays once per chapter. Unlocked in **Part 08** (Ch 49–52); whether at a chapter or a level is open – see [The Nexus → Open decisions](../mechanics/Nexus.md#open-decisions). | Lord Kit: Dardan |
