...
**Dardan's kit is the Nexus.** The four Nexus abilities are unlocked by chapter, not by level – the first arrives at Ch 08, long before the Master promotion – and they are **not equipped and take no Capacity** (a Base class holds 3 and Exchange would not fit). Full rules in [The Nexus](../mechanics/Nexus.md); every value in the [Balancing Guide → The Nexus](../Balancing-Guide.md#nexus). *Active (command)* below is the trigger defined there: it ends the unit's movement for the turn but leaves its action. All four are unusable on a map whose level box locks the band (band shown black).
