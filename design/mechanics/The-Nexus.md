...
Dardan's Lord-Kit system – the four abilities that run over the band between him and his allies: *Exchange*, *Lifeline*, *Heartpulse* and *Earthbound* – what each one does, what it costs, when the band is open and when it is locked, and how the system reads the affinity rank it depends on. This document holds the rules; every value lives in the [Balancing Guide → The Nexus](../Balancing-Guide.md#the-nexus). It is the specification the game is named after.
...
| Core feature | Range and strength scale with affinity rank; the band is golden (open) or black (locked) per chapter; Dardan is the only unit that has it |
...
| Target | *Exchange*, *Lifeline*: one unit of the **player's faction** with which Dardan holds an affinity rank – any rank from D ([Affinity](Affinity.md#8--what-the-nexus-reads)). *Heartpulse*: no target – every unit of the player's faction on the map that holds a rank with Dardan. *Earthbound*: one **enemy** unit. A summoned Solmare beast is never a target of any Nexus ability (*Interaction*) |
...
**Affinity rank.** *Exchange* range, *Lifeline* range and *Heartpulse* strength are read from the **affinity rank between Dardan and the unit** – the five-rank scale D → C → B → A → S of [Affinity](Affinity.md) – as the pair holds it **when the map begins**; points earned during the map do not raise the reach mid-battle. The per-rank tables are in the [Balancing Guide → The Nexus](../Balancing-Guide.md#the-nexus). **Two starting ranks decided by Dardan** and recorded in the [Balancing Guide → Starting ranks](../Balancing-Guide.md#starting-ranks): the eight Vigilant Knights start at **C** with him – they grew up together – and in Ch 08 Dardan holds **B** with Elena ("sie ist ja wie eine Mutter für sie"), which is the rank the Level 08 hostage distance is built on. **Any rank from D reaches.** A unit with which Dardan holds **no** rank cannot be reached by *Exchange* or *Lifeline* at all, and *Heartpulse* does not heal it – no rank, no band ([Affinity → Core Rules → 8](Affinity.md#8--what-the-nexus-reads)). The Nexus feeds the bond back: Exchange, Lifeline and Heartpulse are affinity sources for Dardan and the unit they touch; Earthbound is not.
...
2. He chooses an allied unit whose distance from him is within the **Exchange range for the affinity rank he holds with that unit** ([Balancing Guide → Exchange range by affinity rank](../Balancing-Guide.md#the-nexus)). Distance is counted in tiles and **nothing in between matters** – enemies, walls, doors, magical barriers. The band goes through everything; in [Ch 05](../../story/chapters/Part-01-Path-Of-Liberation/Chapter-05-A-Heart-Of-Gold.md) it goes through the Guardian's barrier.
...
Every value lives in the [Balancing Guide → The Nexus](../Balancing-Guide.md#the-nexus). The rank rows are proposed on the [Affinity](Affinity.md) scale; the rest is open or proposed:
