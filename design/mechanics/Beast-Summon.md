...
**Faction.** A summoned beast belongs to the **Other faction** and is shown in that faction's colour. It is hostile to the enemy and not hostile to the player. It is not a member of the player's army: it is not deployed, does not count against the deployment limit, has no inventory, no supports, no sheet and no name.
...
| ------ | -------- | ------ |
| **Bonds** | Does it strengthen the connections between units? | Yes, by contrast. Every other unit on the field is a person with a bond that the game measures; the beast is the one thing that has none. It obeys within two tiles and runs wild beyond, it cannot be supported, healed or exchanged, and when it dies nobody mourns it. The tamer keeps it close or lets it go – and the game about trust shows, in one unit, what obedience without a bond looks like. |
...
| **The Nexus / Exchange** (backlog) | **A beast cannot be exchanged.** Whatever the Exchange turns out to be, it is Dardan's bond with a person, and the beast is nobody's. Written here so the Exchange specification does not have to ask. |
